...
class checknumberListener(sublime_plugin.ViewEventListener):
    ...
    def on_modified_async(self):
        '''
         работает только если в mcnp файле есть строки
         'Begin Cells' - перед блоком описания ячеек;
         'Begin Surfaces' - перед блоком описания поверхностей;
         'Begin Materials' - перед блоком описания материалов;
         'Begin Tallies' - перед блоком описания оценок;
         'Source Description' - перед блоком описания источника
         '''
        global ch2
        ...
        if ch1 != ch2:
            ...
            # for tallies processing
                ...
                if int(match_t_str) in tallies:
                    v.erase_phantoms("tally_warning")
